...
TTS_API_BASE = os.environ.get("TTS_API_BASE", "http://127.0.0.1:8012").rstrip("/")
STOP = object()
    ...
    while True:
        ...
        try:
            if item is STOP:
                return
            idx, text = item
            log("TTS", f"start idx={idx} chars={len(text)} text={text}")
            t0 = time.perf_counter()
            output = f"/tmp/zipvoice_interactive_{int(time.time() * 1000)}_{idx}.wav"
            r = requests.post(
            ...
            log("TTS", f"synth idx={idx} status={r.status_code} elapsed={synth_dt:.3f}s")
            r.raise_for_status()
            t1 = time.perf_counter()
            play_wav(output)
            play_dt = time.perf_counter() - t1
            log("TTS", f"play idx={idx} elapsed={play_dt:.3f}s total={time.perf_counter() - t0:.3f}s")
...
def main():
    tts_q = queue.Queue()
    th = threading.Thread(target=tts_worker, args=(tts_q,), daemon=True)
    th.start()
    qwen = PersistentQwen()

    print("交互式常驻 Qwen3-VL + ZipVoice 已启动。输入 exit / quit / q / 退出 结束。")
    try:
        while True:
